...
#         control_points[i] = start_point + i / (n_control_points - 1) * (
...
#                         o3d.utility.Vector3dVector(np.array([[x2, y2, z2]]))
#                     )[0]:
#                         continue
...
# def astar_path_spline_fit(
#     start_point, end_point, means, voxel_size=1.0, num_control_points=20, z_range=None
# ):
...
#     bounds = [
#         [min_x - 0.2 * spread_x, max_x + 0.2 * spread_x],
#         [min_y - 0.2 * spread_y, max_y + 0.2 * spread_y],
#         [min_z - 0.2 * spread_z, max_z + 0.2 * spread_z],
#     ]
...
#         fitting_cost += (
#             (curve[i, 0].T - path_arr[i, 0]) ** 2
#             + (curve[i, 1].T - path_arr[i, 1]) ** 2
#             + (curve[i, 2].T - path_arr[i, 2]) ** 2
#         )
...
#     control_points_opt = np.concatenate(
#         (control_points_opt, np.zeros((num_control_points, 1))), axis=1
#     ).T
